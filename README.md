...
An AI-powered platform for academic learning with document analysis, intelligent chat, and flashcard generation. Built with React, TypeScript, Firebase, and Google Gemini AI.
...
**Implementation:** 
- Core chat logic: `src/services/chatService.ts`
- UI component: `src/components/ChatInterface.tsx`
- AI integration: `src/services/firebaseAILogicService.ts`
- Session management: `src/services/firebaseService.ts`
...
**Supported Formats:**
- **PDF**: Text extraction using PDF.js (`src/services/documentProcessor.ts:extractFromPDF`)
- **DOCX**: Text extraction using Mammoth.js (`src/services/documentProcessor.ts:extractFromWord`)
- **PPTX**: Text extraction using PPTX Parser (`src/services/documentProcessor.ts:extractFromPowerPoint`)

**Implementation:**
- Document processor: `src/services/documentProcessor.ts`
- Upload component: `src/components/DocumentUpload.tsx`
...
**Implementation:**
- Flashcard service: `src/services/flashcardService.ts`
- Generation logic: `src/services/flashcardService.ts:generateFlashcards`
- UI component: `src/components/Flashcard.tsx`
- List management: `src/components/FlashcardList.tsx`
- Storage: `src/services/firebaseService.ts` (authenticated) or localStorage (unauthenticated)
...
- Data service: `src/services/firebaseService.ts`
...
**Implementation:**
- Cleanup service: `src/services/automaticCleanupService.ts`
- Database cleanup: `src/services/databaseCleanupService.ts`
- Firebase integration: `src/services/firebaseService.ts:runAutomaticCleanup`
...
│   ├── firebaseAILogicService.ts   # Google Gemini AI integration
...
**Location:** `src/services/documentProcessor.ts`

**Flow:**
1. **File Upload** → User uploads document via `DocumentUpload.tsx` or `ChatInterface.tsx`
...
**Flow:**
1. **Message Input** → User sends message via `ChatInterface.tsx`
...
4. **AI Processing** → Sends to `firebaseAILogicService.generateResponse()`
...
3. **AI Generation** → Sends to `firebaseAILogicService.generateResponse()`
...
**Location:** `src/services/firebaseAuthService.ts` & `src/contexts/AuthContext.tsx`
...
**Location:** `src/services/automaticCleanupService.ts`
...
4. **Cleanup Execution** → `firebaseService.runAutomaticCleanup()`:
...
### AI & Processing
- **Google Gemini API** - AI chat and flashcard generation (gemini-1.5-flash model)
...
4. **Generate flashcards** by asking "Create flashcards from [document]"
5. **Study flashcards** with interactive flip cards and mastery tracking
